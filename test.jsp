...
<%
	...
	state = conn.prepareStatement("select * from sample");
  ...
  <script id="code">
    function init() {
      ...
      function findHeadShot(key) {
        if (key < 0 || key > 16) return "/WebTest/images/HSnopic.jpg"; // There are only 16 images on the server
        return "/WebTest/images/hs" + key + ".jpg"
      }
...
{ "class": "go.TreeModel",
 ...
 <%
  while(rs.next()){
	  if(rs.isLast()){
		  if(rs.getInt("uppergrade") == 0){
 %>
  {"key":<%= rs.getInt("grade")%>, "name":"<%= rs.getString("name")%>", "title":"<%= rs.getString("title")%>"}
<%
		  }
		  else{
			  %>
  {"key":<%= rs.getInt("grade")%>, "name":"<%= rs.getString("name")%>", "title":"<%= rs.getString("title")%>", "parent" : <%= rs.getInt("uppergrade")%> }
			<%
		  }
	  }
	  else{
		  if(rs.getInt("uppergrade") == 0){
		  %>
  {"key":<%= rs.getString("grade")%>, "name":"<%= rs.getString("name")%>", "title":"<%= rs.getString("title")%>"},
		<%
		  }
		  else{
			  %>
